...
$(document).ready(function () {
 ...
 $("#GidListado").dxDataGrid({
        ...
        columns: [
            { dataField: 'idoOrigen', width: '5%', caption: 'Id', alignment: 'center' },
            { dataField: 'nombre', width: '80%', caption: 'Nombre', alignment: 'center' },
            {
                ...
                cellTemplate: function (container, options) {
                    $('<div/>').dxButton({
                        ...
                        onClick: function (e) {
                            var _Ruta = $('#SIM').data('url') + "GestionRiesgo/api/TipoOrigenApi/ObtenerTipoOrigenAsync";
                            $.getJSON(_Ruta,
                                {
                                    id: options.data.idOrigen
                                }).done(function (data) {
                                    if (data !== null) {
            ...
            },
            {
                ...
                cellTemplate: function (container, options) {
                    $('<div/>').dxButton({
                        icon: 'remove',
                        height: 20,
                        hint: 'Eliminar el Tipo de Orígen',
                        onClick: function (e) {
                            var result = DevExpress.ui.dialog.confirm('Desea eliminar el registro seleccionado?', 'Confirmación');
                            result.done(function (dialogResult) {
                                if (dialogResult) {
                                    var _Ruta = $('#SIM').data('url') + "GestionRiesgo/api/TipoOrigenApi/EliminarTipoOrigenAsync";
                                    var params = {
                                        idOrigen: options.data.idOrigen, nombre: "."
                                    };
                                    $.ajax({
                                        type: 'POST',
                                        url: _Ruta,
                                        ...
                                        success: function (data) {
                                            if (data.resp === "Error") DevExpress.ui.dialog.alert('Ocurrió un error ' + data.mensaje, 'Eliminar registro seleccionado');
                                                ...
                                                $('#GidListado').dxDataGrid({ dataSource: EventosDataSource });
...
var txtNombre = $("#txtNombre").dxTextBox({
    ...
    validationRules: [{
        type: "required",
        message: "Debe ingresar el nombre del tipo de orígen!"
    }]
}).dxTextBox("instance");
...
$("#btnGuarda").dxButton({
    ...
    onClick: function () {
        ...
        if (nombre === "") return;
        var params = {
            idOrigen: id, nombre: nombre
        };

        var _Ruta = $('#SIM').data('url') + "GestionRiesgo/api/TipoOrigenApi/GuardarTipoOrigenAsync";
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: _Ruta,
            ...
            success: function (data) {
                if (data.resp === "Error") DevExpress.ui.dialog.alert('Ocurrió un error ' + data.mensaje, 'Guardar Datos');
                    ...
                    $('#GidListado').dxDataGrid({ dataSource: OrigenesDataSource });
...
var popup = $("#PopupNuevo").dxPopup({
        ...
        resizeEnabled: true,
        hoverStateEnabled: true,
        title: "Tipo de Orígen"
    }).dxPopup("instance");

});

var OrigenesDataSource = new DevExpress.data.CustomStore({
    load: function (loadOptions) {
        ...
        $.getJSON($('#SIM').data('url') + 'GestionRiesgo/api/TipoOrigenApi/GetTiposOrigenesAsync', {
