...
$(document).ready(function () {
 ...
 $("#GidListado").dxDataGrid({
        ...
        columns: [
            { dataField: 'idTipoVisita', width: '5%', caption: 'Id', alignment: 'center' },
            { dataField: 'nombre', width: '80%', caption: 'Nombre', alignment: 'center' },
            {
                ...
                cellTemplate: function (container, options) {
                    $('<div/>').dxButton({
                        ...
                        onClick: function (e) {
                            var _Ruta = $('#SIM').data('url') + "GestionRiesgo/api/TiposVisitaApi/ObtenerTipoVisitaAsync";
                            $.getJSON(_Ruta,
                                {
                                    id: options.data.idTipoVisita
                                }).done(function (data) {
                                    if (data !== null) {
                    ...
                    $('<div/>').dxButton({
                        ...
                        onClick: function (e) {
                            ...
                            result.done(function (dialogResult) {
                                if (dialogResult) {
                                    var _Ruta = $('#SIM').data('url') + "GestionRiesgo/api/TiposVisitaApi/EliminarTipoVisitaAsync";
                                    var params = {
                                        idTipoVisita: options.data.idTipoVisita, nombre: "."
                                    };
                                    $.ajax({
                                        type: 'POST',
                                        url: _Ruta,
...
var txtNombre = $("#txtNombre").dxTextBox({
    ...
    validationRules: [{
        type: "required",
        message: "Debe ingresar el nombre del tipo de visita!"
    }]
}).dxTextBox("instance");
...
$("#btnGuarda").dxButton({
    ...
    onClick: function () {
        ...
        if (nombre === "") return;
        var params = {
            idTipoVisita: id, nombre: nombre
        };

        var _Ruta = $('#SIM').data('url') + "GestionRiesgo/api/TiposVisitaApi/GuardarTipoVisitaAsync";
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: _Ruta,
...
var TipoVisitaDataSource = new DevExpress.data.CustomStore({
    load: function (loadOptions) {
        ...
        $.getJSON($('#SIM').data('url') + 'GestionRiesgo/api/TiposVisitaApi/GetTiposVisitaAsync', {
