...
$(document).ready(function () {
 ...
 $("#GidListado").dxDataGrid({
        ...
        columns: [
            { dataField: 'idEvento', width: '5%', caption: 'Id', alignment: 'center' },
            { dataField: 'nombre', width: '80%', caption: 'Nombre', alignment: 'center' },
            {
                ...
                cellTemplate: function (container, options) {
                    $('<div/>').dxButton({
                        ...
                        onClick: function (e) {
                            var _Ruta = $('#SIM').data('url') + "GestionRiesgo/api/EventosApi/ObtenerEventoAsync";
                            $.getJSON(_Ruta,
                                {
                                    id: options.data.idEvento
                                }).done(function (data) {
                                    if (data !== null) {
            ...
            },
            {
                ...
                cellTemplate: function (container, options) {
                    $('<div/>').dxButton({
                        icon: 'remove',
                        height: 20,
                        hint: 'Eliminar el Tipo de Evento',
                        onClick: function (e) {
                            var result = DevExpress.ui.dialog.confirm('Desea eliminar el registro seleccionado?', 'Confirmación');
                            result.done(function (dialogResult) {
                                if (dialogResult) {
                                    var _Ruta = $('#SIM').data('url') + "GestionRiesgo/api/EventosApi/EliminarEventoAsync";
                                    var params = {
                                        idEvento: options.data.idEvento, nombre: "."
                                    };
                                    $.ajax({
                                        type: 'POST',
                                        url: _Ruta,
...
var txtNombre = $("#txtNombre").dxTextBox({
    ...
    validationRules: [{
        type: "required",
        message: "Debe ingresar el nombre del tipo de evento!"
    }]
}).dxTextBox("instance");
...
$("#btnGuarda").dxButton({
    ...
    onClick: function () {
        ...
        if (nombre === "") return;
        var params = {
            idEvento: id, nombre: nombre
        };

        var _Ruta = $('#SIM').data('url') + "GestionRiesgo/api/EventosApi/GuardarEventoAsync";
        $.ajax({
            type: "POST",
            dataType: 'json',
            url: _Ruta,
...
var EventosDataSource = new DevExpress.data.CustomStore({
    load: function (loadOptions) {
        ...
        $.getJSON($('#SIM').data('url') + 'GestionRiesgo/api/EventosApi/GetEventosAsync', {
