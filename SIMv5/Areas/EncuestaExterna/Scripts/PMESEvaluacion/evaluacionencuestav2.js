$(document).ready(function () {
    ...
    $("#btnCalcularCumplimiento").dxButton({
        ...
        type: 'success',
        onClick: function (params) {
            $.getJSON($('#app').data('url') + 'EncuestaExterna/api/PMESEvaluacionApi/CumplimientoContenidoT', {
                ideet: $('#app').data('eet'),
            }).done(function (data) {
                switch (data) {
                    case 'C':
                ...
                var grid = $('#grdPreguntasEvaluacionI').dxDataGrid('instance');
                grid.refresh();
            });
        },
    });
    ...
    $("#grdPreguntasEvaluacionI").dxDataGrid({
        dataSource: grdPreguntasEvaluacionIDataSource,
        allowColumnResizing: true,
        allowSorting: false,
        height: '100%',
        ...
        columns: [
            {
                ...
                cellTemplate: function (cellElement, cellInfo) {
                    ...
                    } else if (cellInfo.data.S_CALCULADO == 'M') {
                        var div = document.createElement("div");
                        div.style.cssText = 'float:right; margin: -4px;';
                        cellElement.get(0).appendChild(div);
    ...
    $("#btnNuevoPE").dxButton({
        icon: 'add',
        onClick: function (params) {
            CargarEstrategia(null, 'PE');
        },
    });

    $("#grdPreguntasEvaluacionPE").dxDataGrid({
        dataSource: grdEstrategiasEvaluacionPEDataSource,
        allowColumnResizing: true,
        allowSorting: false,
        height: '100%',
        ...
        columns: [
            {
                ...
                cellTemplate: function (cellElement, cellInfo) {
                    ...
                    $(div).dxButton({
                        icon: 'edit',
                        onClick: function (params) {
                            CargarEstrategia(cellInfo.data, 'PE');
                        },
                    });
                }
            }, {
                            ...
                            result.done(function (dialogResult) {
                                if (dialogResult) {
                                    $.getJSON(
                                        ...
                                        var grdPreguntasEvaluacionPE = $("#grdPreguntasEvaluacionPE").dxDataGrid("instance");
                                        grdPreguntasEvaluacionPE.option("dataSource", grdEstrategiasEvaluacionPEDataSource);
                                    });
                                }
                            });
                        },
    ...
    $("#btnNuevoEM").dxButton({
        icon: 'add',
        onClick: function (params) {
            CargarEstrategia(null, 'EM');
        },
    });

    $("#grdPreguntasEvaluacionEM").dxDataGrid({
        dataSource: grdEstrategiasEvaluacionEMDataSource,
        allowColumnResizing: true,
        allowSorting: false,
        height: '100%',
        noDataText: 'Adicione las estrategias de movilidad sostenible en tiempos de contingencia formuladas por la organización',
        loadPanel: { text: 'Cargando Datos...' },
        paging: {
            pageSize: 0,
        },
        ...
        columns: [
            {
                ...
                cellTemplate: function (cellElement, cellInfo) {
                    ...
                    $(div).dxButton({
                        icon: 'edit',
                        onClick: function (params) {
                            CargarEstrategia(cellInfo.data, 'EM');
                        },
                    });
                }
            }, {
                            ...
                            result.done(function (dialogResult) {
                                if (dialogResult) {
                                    $.getJSON(
                                        ...
                                        var grdPreguntasEvaluacionEM = $("#grdPreguntasEvaluacionEM").dxDataGrid("instance");
                                        grdPreguntasEvaluacionEM.option("dataSource", grdEstrategiasEvaluacionEMDataSource);
                                    });
                                }
                            });
                        },
...
function CargarEstrategia(datosEstrategia, tipo) {
    ...
    estrategia.option('title', '4. PLANEACIÓN DE ESTRATEGIAS DE MOVILIDAD SOSTENIBLE');
        ...
        items: [
            {
                ...
                label: { text: '1.2.2.VALOR META A ALCANZAR' },
                dataField: 'N_VALOR_META',
...
var grdPreguntasEvaluacionTDataSource = new DevExpress.data.CustomStore({
    load: function (loadOptions) {
        ...
        $.getJSON($('#app').data('url') + 'EncuestaExterna/api/PMESEvaluacionApi/PreguntasEvaluacionv2', {
...
var grdPreguntasEvaluacionIDataSource = new DevExpress.data.CustomStore({
    load: function (loadOptions) {
        var d = $.Deferred();
    ...
    update: function (key, values) {
        $.postJSON(
            $('#app').data('url') + 'EncuestaExterna/api/PMESEvaluacionApi/PreguntasEvaluacionActualizar', { idEncuestaEvaluacion: key.ID_EVALUACION_ENCUESTA, id: key.ID, idPregunta: key.ID_PREGUNTA, tipoRespuesta: key.N_TIPO_RESPUESTA, valor: values.RESPUESTA }
        ).done(function (data) {
            $('#cumplimientoCell').css('background-color', 'gray');
            $('#cumplimiento').text('PENDIENTE');
...
var grdEstrategiasEvaluacionPEDataSource = new DevExpress.data.CustomStore({
    load: function (loadOptions) {
        var d = $.Deferred();
...
var grdEstrategiasEvaluacionEMDataSource = new DevExpress.data.CustomStore({
    load: function (loadOptions) {
        var d = $.Deferred();
