...
function cargarDoc(idpregunta, id) {
...
function AbrirArchivo(idpregunta, id) {
    ...
    var html = '<div class="col-md-12" id="ContenidoArchivo"> <input id="txtDoc_' + id.id + '"  class="form-control" style="width: 140px;"  /><label style="top: -2px;margin-left: 11px;" class="custom-file-upload"><input onchange="cargarDoc(' + idpregunta + ',' + id.id + ')" type="file" name="file1" id="file_' + idpregunta + '" accept="application/pdf" class="btn btn-default"><i class="fa fa-cloud-upload"></i> Examinar</label>'
    html += "<input type='text' style='display:none' value='" + id.id + "' id='txt_id_observacion'>";
    html += '<input type="text" id="fecha" name="fecha" /></div>';
    $("#dialogArchivo").append(html);
    $("#dialogArchivo").dialog();
    ...
    for (var z = 0; z < jsonEncuestas.length; z++) {
        if (jsonEncuestas[z].ESTADO == 1) {
            for (var i = 0; i < jsonEncuestas[z].PREGUNTAS.length; i++) {
                ...
                if (jsonEncuestas[z].PREGUNTAS[i].ID_TIPOPREGUNTA != 3 && jsonEncuestas[z].PREGUNTAS[i].ID_TIPOPREGUNTA != 1 && jsonEncuestas[z].PREGUNTAS[i].ID_TIPOPREGUNTA != 9 && jsonEncuestas[z].PREGUNTAS[i].ID_TIPOPREGUNTA != 11) {
