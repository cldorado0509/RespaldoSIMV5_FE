...
function BuscarEncuestas(Control, Encuesta, Formulario, Responsable, Fecha) {
    try {
        var source =
        {
            ...
            id: 'Codigo',
            url: RutaEncuestas + 'Caracteristicas/BuscarEncuestas?Nombre=' + Encuesta + '&Formulario=' + Formulario + '&Responsable=' + Responsable + '&Fecha=' + Fecha
        };
        ...
        $("#" + Control).jqxGrid(
                {
                    ...
                    columns: [
                        ...
                        { text: 'Nombre', datafield: 'NombreEncuesta', width: '50%' },
                        {
                            text: 'Accion', width: '35%', cellsrenderer: function () {
                                return "<div id='divActions' valign='center' align='center'> \
                                                <a onclick='ClonarEncuesta();' class='btn btn-primary btn_left'><span class='glyphicon glyphiconButton glyphicon-share'></span><label class='txt_hide'>&nbsp;Replicar</label></a> \
                                                <a onclick='EditarEncuesta();' class='btn btn-primary btn_left'><span class='glyphicon glyphiconButton glyphicon-edit'></span><label class='txt_hide'>&nbsp;Editar</label></a> \
                                        </div>";
                            }
                        }
                    ]
        ...
        if (dataAdapter.records.length == 0) {
            alert('No se hallaron datos que cumplieran con el criterio dado.');
        }
    ...
    try {
        var source =
        {
            ...
            datafields: [
                ...
                { name: 'Requerida', type: 'boolean' },
            ],
            id: 'Codigo',
            url: RutaEncuestas + 'Utilidades/BuscarPreguntas?Pregunta=' + Pregunta + '&Encuesta=' + Encuesta + '&Responsable=' + Responsable + '&Fecha=' + Fecha
        };
...
function CargarPreguntasTodas(Control, Vincular, TipoEncuesta) {
    try {
        var source =
        {
            async: false,
            ...
            id: 'Codigo',
            url: RutaEncuestas + 'Preguntas/ObtenerPreguntastodas?Vincular=' + Vincular + '&TipoEncuesta=' + TipoEncuesta
        };
...
function CargarEncuestasTodas(Control) {
    try {
        var source =
        {
            ...
            id: 'ID_ENCUESTA',
            url: RutaEncuestas + 'Caracteristicas/ObtenerEncuestas'
        };
        ...
        $("#" + Control).jqxGrid(
        {
            ...
            columns: [
                ...
                { text: 'Caracteristica', datafield: 'S_NOMBRE', width: '50%' },
...
function ClonarEncuesta() {
    AbrirPopUp('Replicar Caracteristica', RutaEncuestas + 'Caracteristicas/clonar/' + CodigoEncuestaSeleccionada, true, 500, 'Replicar');
}

function EditarEncuesta() {
    AbrirPopUp('Editar Caracteristica', RutaEncuestas + 'Caracteristicas/edit/' + CodigoEncuestaSeleccionada, true, 500, 'Guardar Cambios');
}
    ...
    if (Peso <= 100) {
        $.ajax({
            ...
            success: function (result) {
                ...
                if (datos == "Ok") {
                    Respuesta = datos;
                    //window.parent.$('#VincularPregunta').attr('disabled', 'disabled');
...
{    
    ...
    $.ajax({
        ...
        async: false,
        type: "POST",
        url: RutaEncuestas + "Caracteristicas/CrearEncuesta?Nombre=" + Nombre + "&Descripcion=" + Descripcion + "&CodigoFormulario=" + Formulario + "&TipoEncuesta=" + Tipo,
        contentType: "application/json; charset=utf-8",
        success: function (result) {
            Datos = result;
        }
